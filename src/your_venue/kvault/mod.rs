mod account_metas;
mod math;
mod state;
mod update;

pub use account_metas::FARMS_PROGRAM_ID;
pub use state::KvaultState;

use solana_instruction::Instruction;
use solana_pubkey::Pubkey;

use crate::{
    account_caching::AccountsCache,
    your_venue::state::{GlobalConfig, WrapperVault},
    trading_venue::error::TradingVenueError,
};

pub const KVAULT_PROGRAM_ID: Pubkey =
    Pubkey::from_str_const("KvauGMspG5k6rtzrqqn7WNn3oZdyKqLKwK2XWQ8FLjd");

pub const DEPOSIT_IX_DISCRIMINATOR: [u8; 8] = [0x9e, 0xa8, 0x14, 0xa4, 0x47, 0x9b, 0x1c, 0xa3];
pub const WITHDRAW_IX_DISCRIMINATOR: [u8; 8] = [0xe0, 0x69, 0xdf, 0xf4, 0xc5, 0xfa, 0x41, 0x3e];

#[derive(Clone, Default)]
pub struct Kvault {
    state: KvaultState,
    current_slot: u64,
    unix_timestamp: i64,
}

impl Kvault {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn required_pubkeys(&self, wv: &WrapperVault) -> Vec<Pubkey> {
        update::required_pubkeys(&self.state, wv)
    }

    pub async fn update(
        &mut self,
        wv: &WrapperVault,
        cache: &dyn AccountsCache,
    ) -> Result<(), TradingVenueError> {
        update::run(
            &mut self.state,
            &mut self.current_slot,
            &mut self.unix_timestamp,
            wv,
            cache,
        )
        .await
    }

    pub fn quote_deposit(
        &self,
        wv: &WrapperVault,
        gc: &GlobalConfig,
        amount: u64,
    ) -> Result<u64, TradingVenueError> {
        math::quote_deposit(
            &self.state,
            self.current_slot,
            self.unix_timestamp,
            wv,
            gc,
            amount,
        )
    }

    pub fn quote_withdraw(
        &self,
        wv: &WrapperVault,
        amount: u64,
    ) -> Result<u64, TradingVenueError> {
        math::quote_withdraw(
            &self.state,
            self.current_slot,
            self.unix_timestamp,
            wv,
            amount,
        )
    }

    pub fn build_deposit_ix(
        &self,
        wv: &WrapperVault,
        user: Pubkey,
        in_amount: u64,
        min_out: u64,
    ) -> Result<Instruction, TradingVenueError> {
        let accounts = account_metas::build_deposit(&self.state, wv, &user)?;
        let mut data = Vec::with_capacity(24);
        data.extend_from_slice(&DEPOSIT_IX_DISCRIMINATOR);
        data.extend_from_slice(&in_amount.to_le_bytes());
        data.extend_from_slice(&min_out.to_le_bytes());
        Ok(Instruction {
            program_id: crate::your_venue::OVERPASS_PROGRAM_ID,
            accounts,
            data,
        })
    }

    pub fn build_withdraw_ix(
        &self,
        wv: &WrapperVault,
        user: Pubkey,
        in_amount: u64,
        min_out: u64,
    ) -> Result<Instruction, TradingVenueError> {
        let accounts = account_metas::build_withdraw(&self.state, wv, &user)?;
        let mut data = Vec::with_capacity(24);
        data.extend_from_slice(&WITHDRAW_IX_DISCRIMINATOR);
        data.extend_from_slice(&in_amount.to_le_bytes());
        data.extend_from_slice(&min_out.to_le_bytes());
        Ok(Instruction {
            program_id: crate::your_venue::OVERPASS_PROGRAM_ID,
            accounts,
            data,
        })
    }

    pub fn lookup_table_keys(&self, wv: &WrapperVault) -> Vec<Pubkey> {
        let mut keys = vec![
            KVAULT_PROGRAM_ID,
            crate::your_venue::klend::KLEND_PROGRAM_ID,
            account_metas::FARMS_PROGRAM_ID,
            wv.source_pool,
            self.state.token_vault,
            self.state.base_vault_authority,
        ];
        for alloc in self.state.active_allocations.iter() {
            keys.push(alloc.reserve);
            keys.push(alloc.ctoken_vault);
        }
        keys
    }
}
