mod account_metas;
mod math;
mod state;
mod update;

pub use state::MarginfiState;

use solana_instruction::Instruction;
use solana_pubkey::Pubkey;

use crate::{
    account_caching::AccountsCache,
    your_venue::state::{GlobalConfig, WrapperVault},
    trading_venue::error::TradingVenueError,
};

pub const MARGINFI_PROGRAM_ID: Pubkey =
    Pubkey::from_str_const("MFv2hWf31Z9kbCa1snEPYctwafyhdvnV7FZnsebVacA");

pub const DEPOSIT_IX_DISCRIMINATOR: [u8; 8] = [0x67, 0x07, 0xd7, 0x7b, 0xb2, 0xf4, 0xb9, 0xa3];
pub const WITHDRAW_IX_DISCRIMINATOR: [u8; 8] = [0xd5, 0x85, 0x8d, 0x5d, 0xf2, 0x3a, 0xf0, 0x85];

#[derive(Clone, Default)]
pub struct Marginfi {
    state: MarginfiState,
    unix_timestamp: i64,
}

impl Marginfi {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn required_pubkeys(&self, wv: &WrapperVault) -> Vec<Pubkey> {
        update::required_pubkeys(wv)
    }

    pub async fn update(
        &mut self,
        wv: &WrapperVault,
        cache: &dyn AccountsCache,
    ) -> Result<(), TradingVenueError> {
        update::run(&mut self.state, &mut self.unix_timestamp, wv, cache).await
    }

    pub fn quote_deposit(
        &self,
        wv: &WrapperVault,
        gc: &GlobalConfig,
        amount: u64,
    ) -> Result<u64, TradingVenueError> {
        math::quote_deposit(&self.state, self.unix_timestamp, wv, gc, amount)
    }

    pub fn quote_withdraw(
        &self,
        wv: &WrapperVault,
        amount: u64,
    ) -> Result<u64, TradingVenueError> {
        math::quote_withdraw(&self.state, self.unix_timestamp, wv, amount)
    }

    pub fn build_deposit_ix(
        &self,
        wv: &WrapperVault,
        user: Pubkey,
        in_amount: u64,
        min_out: u64,
    ) -> Result<Instruction, TradingVenueError> {
        let accounts = account_metas::build_deposit(&self.state, wv, &user)?;
        let mut data = Vec::with_capacity(24);
        data.extend_from_slice(&DEPOSIT_IX_DISCRIMINATOR);
        data.extend_from_slice(&in_amount.to_le_bytes());
        data.extend_from_slice(&min_out.to_le_bytes());
        Ok(Instruction {
            program_id: crate::your_venue::OVERPASS_PROGRAM_ID,
            accounts,
            data,
        })
    }

    pub fn build_withdraw_ix(
        &self,
        wv: &WrapperVault,
        user: Pubkey,
        in_amount: u64,
        min_out: u64,
    ) -> Result<Instruction, TradingVenueError> {
        let accounts = account_metas::build_withdraw(&self.state, wv, &user)?;
        let mut data = Vec::with_capacity(24);
        data.extend_from_slice(&WITHDRAW_IX_DISCRIMINATOR);
        data.extend_from_slice(&in_amount.to_le_bytes());
        data.extend_from_slice(&min_out.to_le_bytes());
        Ok(Instruction {
            program_id: crate::your_venue::OVERPASS_PROGRAM_ID,
            accounts,
            data,
        })
    }

    pub fn lookup_table_keys(&self, wv: &WrapperVault) -> Vec<Pubkey> {
        let mut keys = vec![
            MARGINFI_PROGRAM_ID,
            wv.source_pool,
            wv.source_position_pda,
            self.state.liquidity_vault,
        ];
        for k in self.state.oracle_keys.iter() {
            if *k != Pubkey::default() {
                keys.push(*k);
            }
        }
        keys
    }
}
